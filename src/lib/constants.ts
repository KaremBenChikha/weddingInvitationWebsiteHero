export const COUPLE = {
  latin: "Aicha & Zakaria",
  arabic: "عائشة و زكرياء",
};

export const WEDDING = {
  date: "11.07.2026",
  dateTime: "11 Juillet 2026 · 14h00",
  dateLabelAr: "١١ يوليو ٢٠٢٦ · ١٤:٠٠",
  location: "Trois-Rivières, Québec",
  locationAr: "تروا ريفيير، كيبيك",
  address: "187 rue Bourassa, Trois-Rivières QC G9B 1C4, Canada",
};

export const TIMELINE = [
  { time: "14:00", labelFr: "Cérémonie", labelAr: "مراسم الزفاف" },
  { time: "15:00", labelFr: "Cocktail", labelAr: "كوكتيل" },
  { time: "16:30", labelFr: "Photos", labelAr: "جلسة تصوير" },
  { time: "17:30", labelFr: "Dîner", labelAr: "العشاء" },
  { time: "19:00", labelFr: "Première danse", labelAr: "الرقصة الأولى" },
  { time: "20:00", labelFr: "Fête & soirée", labelAr: "الحفل والسهرة" },
];

export const CONTACT = {
  phone: "+1 819 555 0123",
};

export const CONTENT = {
  popupTitle: "Vous êtes invité·e",
  popupTitleAr: "تمت دعوتكم",
  popupSubtitle: "Au mariage d'Aicha & Zakaria",
  popupSubtitleAr: "لحفل زفاف عائشة و زكرياء",
  popupCta: "Découvrir les détails",
  popupCtaAr: "اكتشف التفاصيل",
  invitationLine1: "Nous vous invitons à notre mariage",
  invitationLine1Ar: "ندعوكم لحضور حفل زفافنا",
  invitationLine2: "Célébrons ensemble le plus beau jour de notre vie",
  invitationLine2Ar: "لنحتفل معًا بأجمل يوم في حياتنا",
  invitationBody: "Chers amis, chère famille,\n\nAlors que nous nous apprêtons à dire \"oui\", nous sommes remplis de gratitude pour les personnes formidables qui nous entourent. Votre présence à nos côtés en ce jour si spécial serait le plus beau des cadeaux.\n\nAvec tout notre amour,",
  invitationBodyAr: "أصدقاؤنا الأعزاء، عائلتنا الكريمة،\n\nبينما نستعد لنقول \"نعم\"، نشعر بالامتنان لكل الأشخاص الرائعين في حياتنا. حضوركم إلى جانبنا في هذا اليوم المميز سيكون أجمل هدية.\n\nمع كل حبنا،",
  timelineTitle: "Déroulement de la journée",
  timelineTitleAr: "برنامج اليوم",
  contactTitle: "Des questions ?",
  contactTitleAr: "أسئلة؟",
  dressCodeTitle: "Tenue suggérée",
  dressCodeTitleAr: "اللباس المقترح",
  dressCodeBody: "Tenue de cérémonie élégante. Nous vous invitons à porter des tons chauds et dorés pour célébrer avec nous.",
  dressCodeBodyAr: "ملابس أنيقة. ندعوكم لارتداء ألوان دافئة وذهبية للاحتفال معنا.",
  rsvpTitle: "Confirmez votre présence",
  rsvpTitleAr: "تأكيد الحضور",
  rsvpSubtitle: "Merci de confirmer avant le 15 Juin 2026",
  rsvpSubtitleAr: "يرجى التأكيد قبل ١٥ يونيو ٢٠٢٦",
  rsvpNameLabel: "Votre nom",
  rsvpNameLabelAr: "اسمك",
  rsvpGuestsLabel: "Nombre de personnes",
  rsvpGuestsLabelAr: "عدد الأشخاص",
  rsvpMessageLabel: "Un message ? (optionnel)",
  rsvpMessageLabelAr: "رسالة؟ (اختياري)",
  rsvpSubmit: "Envoyer",
  rsvpSubmitAr: "إرسال",
  rsvpSuccess: "Merci ! Votre confirmation a bien été envoyée.",
  rsvpSuccessAr: "شكراً! تم إرسال تأكيدكم بنجاح.",
  rsvpError: "Une erreur est survenue. Veuillez réessayer.",
  rsvpErrorAr: "حدث خطأ. يرجى المحاولة مرة أخرى.",
  galleryTitle: "Notre histoire",
  galleryTitleAr: "قصتنا",
  scratchTitle: "La date",
  scratchTitleAr: "التاريخ",
  scratchHint: "✦ Grattez pour découvrir ✦",
  scratchHintAr: "✦ إمسح لتكتشف ✦",
  scratchReveal: "Vous êtes invité·e !",
  scratchRevealAr: "!تمت دعوتكم",
  scratchDay: "11",
  scratchMonth: "Juillet",
  scratchYear: "2026",
  footerText: "Aicha & Zakaria",
};

export const DRESS_CODE_COLORS = [
  { hex: "#d4a843", label: "Or" },
  { hex: "#e8d5a3", label: "Champagne" },
  { hex: "#c4825a", label: "Terracotta" },
  { hex: "#8b6f5e", label: "Taupe" },
  { hex: "#f5efe4", label: "Crème" },
];

export const RSVP_URL = "https://script.google.com/macros/s/AKfycbz-cDjVgIGhs0DihcZMK0MBrBY3JH048t1eFwFWMRmwpYnxEVUJkq3-ioR6oYFDiJoX/exec";
